...
def crawl_apk(device_id, apk_path, apk_queue, login_signal_dir):
    ...
    try:
        logging.info(f"Starting to crawl APK: {apk_name} on device: {device_id}")
        cmd = [
            "droidbot",
            "-d", device_id,
            "-a", apk_path,
...
def device_worker(device_id, apk_queue, login_signal_dir):
    while not apk_queue.empty() and not stop_event.is_set():
        try:
            ...
            crawl_apk(device_id, apk_path, apk_queue, login_signal_dir)
        except queue.Empty:
